...
const Cart: React.FC<IModal> = (props) => {
  ...
  const qtyHandler = (e: any, id: number) => {
    ...
    const updatedCart =
      e.target.id === "minusQty"
        ? cartCtx.map((item: any) => (item.id === id && item.qty > 1 ? { ...item, qty: item.qty - 1 } : item))
        : cartCtx.map((item: any) => (item.id === id ? { ...item, qty: item.qty + 1 } : item));
          ...
          <>
            <div className={styles.topRow}>
              <p className={styles.cartQty}>
                cart <span>({cartCtx.length === 0 ? 0 : cartCtx.length})</span>
              </p>
              ...
              {cartCtx.map((item: any) => (
                <div className={styles.productItem} key={item.id}>
                  <div className={styles.imageContainer}>
                    <Image src={item.image} alt={item.product} layout="responsive" width="150" height="150" />
                  </div>
                  <div>
            ...
            <div className={styles.totalRow}>
              ...
              <p className={styles.total}>{`$ ${total.toLocaleString("en-US")}`}</p>
            </div>
            <Link href="/checkout">
              <button className={`button buttonLight ${styles.checkoutButton}`} onClick={closeModalHandler}>
                checkout
              </button>
            </Link>
          </>
